...
jsPsych.plugins['symmetry-judgement-task'] = (function () {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      button_html: {
        ...
        default:
          '<button class="jspsych-btn-numpad" style = "width:120px; font-size: 16px" >%choice%</button>',
        array: true,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    const blackBoxNumber = jsPsych.randomization.sampleWithoutReplacement(
      trial.number_darkened,
      1
    );
    let blackBoxes = jsPsych.randomization.sampleWithoutReplacement(
      matrix,
      blackBoxNumber
    );

    // creating identical of matrix, but with black boxes removed:
    for (let i = 0; i < blackBoxNumber; i++) {
      h = blackBoxes[i];
      const id = matrix.indexOf(h);
      matrix.splice(id, 1);
    }

    // creating symmetric boxes:
    const blackBoxesSymmetric = [];
    for (let i = 0; i < blackBoxNumber; i++) {
      h = 7 - blackBoxes[i][1];
    ...
    var pSymmetric = jsPsych.randomization.sampleWithReplacement([0, 1], 1, [
      trial.proportion,
      1 - trial.proportion,
    ])[0];
    ...
    if (pSymmetric == 1) {
      const numberBoxesDifferent =
        jsPsych.randomization.sampleWithoutReplacement([3, 4], 1)[0];
      var boxes = jsPsych.randomization.sampleWithoutReplacement(
        blackBoxes,
        blackBoxNumber - numberBoxesDifferent
      );
      var boxesAdd = jsPsych.randomization.sampleWithoutReplacement(
        matrix,
        numberBoxesDifferent
      );
      blackBoxes = [];
    ...
    const paper_size = 4 * 2 * trial.size_cells;
    ...
    for (let i = 0; i < trial.choices.length; i++) {
      ...
      html +=
        '<div class="jspsych-html-button-response-button"  style="display: inline-block; margin:20px 30px" id="jspsych-html-button-response-button-' +
        i +
        '" data-choice="' +
        i +
        '">' +
