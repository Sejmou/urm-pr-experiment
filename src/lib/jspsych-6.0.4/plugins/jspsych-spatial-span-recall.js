...
jsPsych.plugins['spatial-span-recall'] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    var divArray = [];
    var ttArray = [];
    globalThis.recordClick = function (data) {
    ...
    globalThis.clearSpace = function () {
      if (nRecalled > 0) {
    ...
    const paper_size = grid * trial.size_cells;

    display_element.innerHTML =
      '<div style="font-size:24">RECALL<br><br></div>';
    ...
    for (let i = 0; i < matrix.length; i++) {
      paper.innerHTML +=
        '<div class="jspsych-btn-grid" style="position: absolute; top:' +
        matrix[i][0] * (trial.size_cells - 3) +
        'px; left:' +
        ...
        nothing +
        '</div>'; // <img src="img/whitegrid.png" style= "width:'+trial.size_cells+'px; height:'+trial.size_cells+'px;"/img></button></div>';
    }
