...
jsPsych.plugins['spatial-span'] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    let red_box = trial.selected_box;
    ...
    for (var i = 0; i < matrix.length; i++) {
      paper.innerHTML +=
        '<div class="jspsych-grid" style="position: absolute; top:' +
        matrix[i][0] * (trial.size_cells - 3) +
        'px; left:' +
        matrix[i][1] * (trial.size_cells - 3) +
        'px";><div class="whiteBox" /div></div>';
    }

    if (trial.display_red_box) {
      paper.innerHTML +=
        '<div class="jspsych-grid" style="position: absolute; top:' +
        red_box[0] * (trial.size_cells - 3) +
        'px; left:' +
        red_box[1] * (trial.size_cells - 3) +
        'px";><div class="redBox" /div></div>';
    }
