...
jsPsych.plugins['survey-text'] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    for (let i = 0; i < trial.questions.length; i++) {
      ...
      html +=
        '<p class="jspsych-survey-text">' + trial.questions[i].prompt + '</p>';
      var autofocus = i == 0 ? 'autofocus' : '';
      if (trial.questions[i].rows == 1) {
        html +=
          '<input type="text" name="#jspsych-survey-text-response-' +
          i +
          '" size="' +
    ...
    // add submit button
    html +=
      '<button id="jspsych-survey-text-next" class="jspsych-btn jspsych-survey-text">' +
      trial.button_label +
      '</button>';
